...
jsPsych.plugins["rok"] = (function () {
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            stimulus_type: {
                ...
                default: 0,
                description: "Apperance of stimulus (0-triangles, 1-circle, 2-square, 3-origami_birds, 4-image)"
            },
            aperture_shape: {
                type: jsPsych.plugins.parameterType.INT,
    ...
    plugin.trial = function (display_element, trial) {
        ...
        trial.units = assignParameterValue(trial.units, plugin.info.parameters.experiment_main_task.units);
        ...
        if (trial.prompt != null && !Array.isArray(trial.prompt)) {
            let p = document.createElement('div');
            p.style.margin = '10px';
        ...
        for (let i = 0; i < nApertures; i++) {
            ...
            for (let j = 0; j < nOob; j++) {
                ...
                if (stimulusType === 0) {
                    oob = new Oob(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i]);
                } else if (stimulusType === 1) {
                    oob = new OobCircle(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i]);
                } else if (stimulusType === 2) {
                    oob = new OobSquare(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i]);
                } else if (stimulusType === 3) {
                    oob = new OobBird(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i]);
                    ...
                    oob = new OobImage(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, imageArray, keyframes, keyframeTime, mirrorType, canvasArray[i], ctxArray[i]);
                }
                oobs.push(oob);
            }
...
/// O(rientated) ob(ject)
class Oob {
    constructor(size, color, orientation, movementDirection, speed, randomisation, apertureType, isRandomWalk, isRandomOrientated, isFade, canvas, ctx) {
        this.canvas = canvas;
        if (trial.units === 'px') {
...
class OobBird extends Oob {
    constructor(size, color, orientation, movementDirection, speed, randomisation, apertureType, isRandomWalk, isRandomOrientated, isFade, canvas, ctx) {
        super(size, color, orientation, movementDirection, speed, randomisation, apertureType, isRandomWalk, isRandomOrientated, isFade, canvas, ctx);
...
class OobImage extends Oob {
    constructor(size, color, orientation, movementDirection, speed, randomisation, apertureType, isRandomWalk, isRandomOrientated, isFade, imageArray, keyframes, keyframeTime, mirrorType, canvas, ctx) {
        super(size, color, orientation, movementDirection, speed, randomisation, apertureType, isRandomWalk, isRandomOrientated, isFade, canvas, ctx);
